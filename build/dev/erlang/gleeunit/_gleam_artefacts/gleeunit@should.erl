...
-spec be_ok({ok, IAI} | {error, any()}) -> IAI.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, IAN}) -> IAN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(IAQ)) -> IAQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
