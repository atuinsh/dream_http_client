...
-spec be_ok({ok, HZE} | {error, any()}) -> HZE.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, HZJ}) -> HZJ.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(HZM)) -> HZM.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
